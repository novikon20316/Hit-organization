      ...
      if (isSupervisor && previousScore !== null) {
        ...
        await Promise.all(studentIds.map((studentId) =>
          notifyUser({
            ...
            pushBodyHe: `המנחה עדכן את הציון עבור "${milestoneTitle.he}". פתח/י את האפליקציה לפרטים.`,
            pushBodyEn: `Your supervisor updated the grade for "${milestoneTitle.en}". Open the app for details.`,
            relatedProjectId: projectId ?? null,
            relatedMilestoneId: milestoneId,
