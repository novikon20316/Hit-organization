...
export const importUsersAdmin = async (req: AuthenticatedRequest, res: Response) => {
  if (req.user?.role !== 'system_admin') {
    return res.status(403).json({ message: 'Access denied: system_admin only.' });
  }
  ...
  try {
    const summary = await importUsersFromBuffer(file.buffer, { lang: 'he' });
    return res.status(200).json({ success: true, summary });
  } catch (error: any) {
    console.error('importUsersAdmin error:', error);
...
// ─── POST /api/coordinator/users/import ───────────────────────────────────────
// Rows outside the coordinator's own faculty are skipped (not failed) and
// reported individually — the coordinator for CS cannot register Engineering
// students, and vice versa.
export const importUsersCoordinator = async (req: AuthenticatedRequest, res: Response) => {
  const isCoordinator = req.user?.role === 'coordinator' || req.user?.roles?.includes('coordinator');
  if (!isCoordinator) {
    return res.status(403).json({ message: 'Access denied: coordinator only.' });
  }
