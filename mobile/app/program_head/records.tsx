// app/program_head/records.tsx
// Entry point for the master's program head's "Project Records"
// drill-down: lists every supervisor within this program head's own scope
// (the server resolves that scope — see
// apiClient.getScopedSupervisorsForRecords() and
...
export default function ProgramHeadRecordsScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F8FA' }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        <Pressable
          onPress={() => (router.canGoBack() ? router.back() : router.replace('/program_head/program_head_dashboard' as any))}
        ...
        {!loading && !error && supervisors.map((sup) => (
          <Pressable
            key={sup.id}
            onPress={() => router.push({ pathname: '/program_head/records/[supervisorId]', params: { supervisorId: sup.id, lang } } as any)}
