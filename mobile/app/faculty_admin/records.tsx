...
// Entry point for the faculty admin's "Project Records" drill-down: lists
// every supervisor within this faculty admin's own faculty scope (the
// server resolves that scope — see
...
export default function FacultyAdminRecordsScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: ap.surface }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        <Pressable
          onPress={() => (router.canGoBack() ? router.back() : router.replace('/faculty_admin/dashboard' as any))}
        ...
        {!loading && !error && supervisors.map((sup) => (
          <Pressable
            key={sup.id}
            onPress={() => router.push({ pathname: '/faculty_admin/records/[supervisorId]', params: { supervisorId: sup.id, lang } } as any)}
