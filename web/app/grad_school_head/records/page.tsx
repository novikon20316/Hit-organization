...
// app/grad_school_head/records/page.tsx
// Entry point for the grad school head's project-records drill-down —
// lists every supervisor within the grad school head's own faculty scope
...
const GRAD_SCHOOL_HEAD_ROLES: AppRole[] = ['grad_school_head'];

interface SupervisorSummary {
  id: string; displayName: string; email: string; facultyId: string;
}

export default function GradSchoolHeadRecordsPage() {
  const { isAllowed } = useRequireRole(GRAD_SCHOOL_HEAD_ROLES);
      ...
      <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6">
        ...
        <div className="grid gap-2">
          {supervisors?.map((s) => (
            <Link
              key={s.id}
              href={`/grad_school_head/records/${s.id}`}
