  ...
  return (
    <DashboardShell title={lang === 'he' ? 'רישומי הפרויקטים של המנחה' : 'Supervisor Project Records'}>
      <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6">
        ...
        {!error && projects === null && <p className="text-sm text-program-head-on-surface-variant" role="status" aria-live="polite">{lang === 'he' ? 'טוען…' : 'Loading…'}</p>}
        ...
        <div className="grid gap-2">
          {projects?.map((p) => (
            ...
            >
              <p className="text-sm font-semibold text-program-head-on-surface">{lang === 'he' ? p.titleHe || p.titleEn : p.titleEn || p.titleHe}</p>
