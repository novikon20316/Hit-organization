...
// app/coordinator/records/page.tsx
// Entry point for the coordinator's project-records drill-down — lists every
// supervisor within the coordinator's own faculty scope (see
// GET /api/project-records/supervisors). Each row drills into
...
const COORDINATOR_ROLES: AppRole[] = ['coordinator'];

interface SupervisorSummary {
  id: string; displayName: string; email: string; facultyId: string;
}

export default function CoordinatorRecordsPage() {
  const { isAllowed } = useRequireRole(COORDINATOR_ROLES);
