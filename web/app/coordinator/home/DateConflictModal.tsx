...
export function DateConflictModal({ milestone, examiners, onClose, onResolved }: DateConflictModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      ...
      >
        <h2 className="text-lg font-semibold text-coordinator-on-surface">⚠️ {lang === 'he' ? 'לא נמצא תאריך משותף' : 'No common date found'}</h2>
        <p className="mt-2 text-sm text-coordinator-on-surface-variant">
          {lang === 'he'
        ...
        <button
          ...
          disabled={saving}
          className="mt-4 w-full rounded-lg bg-coordinator-primary px-3.5 py-2.5 text-sm font-semibold text-coordinator-on-primary hover:bg-coordinator-primary-container disabled:opacity-60"
        >
          {saving ? '…' : lang === 'he' ? '1️⃣ שמור בוחנים ובחר תאריך אוטומטית' : '1️⃣ Keep examiners & auto-pick a date'}
        </button>

        <p className="mt-5 text-sm font-medium text-coordinator-on-surface">{lang === 'he' ? '2️⃣ או החלף בוחן:' : '2️⃣ Or replace an examiner:'}</p>
          ...
          {panel.map((member) => {
            ...
            return (
              <button
                ...
                className={`rounded-lg border px-3 py-2 text-start text-sm ${active ? 'border-coordinator-primary bg-coordinator-primary text-coordinator-on-primary' : 'border-coordinator-outline-variant bg-coordinator-surface-container-low text-coordinator-on-surface'}`}
              >
        ...
        <div className="mt-3 flex gap-1 rounded-full bg-coordinator-surface-container-low p-0.5" style={{ width: 'fit-content' }}>
          {(['internal', 'external'] as const).map((t) => (
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                replacementType === t ? 'bg-coordinator-primary text-coordinator-on-primary' : 'text-coordinator-on-surface-variant'
              }`}
            >
