...
//
// Client-side mirror of server/src/services/prerequisites.ts — BrowseProjects.tsx
// reads project docs straight off a live Firestore listener (see
// useStudentData.ts), not through a server endpoint, so there's no backend
// layer to normalize legacy data on the way out. A project created before
// minGrade shipped still has `prerequisites` as a plain string[] in
// Firestore; normalizePrerequisites accepts both shapes so this component
