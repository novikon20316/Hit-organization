...
export interface ExaminerTokenDoc {
  ...
  // see app/examiner-access's DataScienceExaminerEvaluationForm) — the SAME
  // two-rubric shape internal examiners get via AssignedMilestone, denormalized
  // at token-creation time (see server/src/services/examinerAccess.ts).
  // Absent for every other faculty's token, which keeps using
  // gradingComponents/OpinionForm exactly as today.
